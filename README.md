...
- Editorial photos are inventoried in `src/data/media.ts`. Display dimensions account for EXIF rotation, mosaic tiles derive their frame from those dimensions and preserve the full image by default, and the home hero uses per-slide full-frame rendering. Inventory `caption` fields stay hidden until Dr. Njo saves a reviewed caption in the dashboard Photos tab (`GET/PATCH /api/photo-captions`). Saved captions render on the profile mosaic and lightbox via `qaCaptions`. Hero slides, community posts, resources, and the interview quote image stay caption-free. Do not render name bars or `PhotoNameOverlay`. Keep accurate `alt` text. `npm run check:media` fails if inventory captions return to those surfaces. The interview player is defined in `src/data/interview-video.ts`. News-tab assets in `public/media/` include the Diana Fat Board of Regents congratulations, The Practice Blueprint dinner recap, the Another perfect match photos, the Beyond the Chair flyer, and Panel of Experts dinner photos. The September 2026 set (`sharedUpdateImages`) adds the July 2026 San Francisco seminar, Dugoni Business Club photos, Backstage Retreat/Dallas photos, the FOUND book launch, the Dental Lifestyles Magazine feature, and the handbook second-edition Coming Soon graphic; the same photos and testimonials are published on practicetransitionsinstitute.com.
...
- Event/news surfaces are date-aware, refresh hourly, exclude completed events from current schema, and hand current registration intent to PTI. The October 2, 2026 Sacramento seminar stays on the existing Mastering Your Dental Transition calendar date at TDIC Headquarters, with an occurrence-only flyer and date-gated early-bird pricing.
...
- `/resources/[slug]` (generated from resource article data)
  ...
  - `/resources/second-book` (upcoming expanded second edition of *Dental Practice Transitions Handbook*; foreword by Dr. Glenn Vo; no release date published)
- `/dentalflix`
- `/phillips-event`
- `/contact`
- `/contact/success`
...
The backend endpoint constants live in `src/config/form-backends.ts`, and `npm run check:forms` guards them as part of `npm run check:parity`.
