...
Profile gallery captions and confirmed names may still exist on `EditorialMediaAsset` in `src/data/media.ts` for inventory, but they are not rendered. Images stand on their own because some labels have not matched the photos. Do not reintroduce name bars (`PhotoNameOverlay`) or figcaptions under photos. The tuxedo-and-medallion portrait alt text names Dr. Allen Budenz. The office leadership photo alt names Nader Nadershahi without naming the startup. `scripts/check-media.ts` asserts UI components do not render caption text.
...
The upcoming book is the expanded second edition of *Dental Practice Transitions Handbook* (`/resources/second-book`, `resource-articles.ts`): same title and subtitle, new foreword by Dr. Glenn Vo, new advisor material, new appendices. `heroImage` on the article renders the Coming Soon graphic, and `/resources` shows the same pictorial above the book/PTI cards via `secondEditionAnnouncementImage`. No release date is published; do not invent one.
